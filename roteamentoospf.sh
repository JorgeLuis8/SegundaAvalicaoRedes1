...
# Passo 9: Configurar OSPF com FRR nos Roteadores
echo "### Passo 9: Configurando OSPF com FRR nos Roteadores ###"

FRR_DAEMONS_CONFIG_OSPF='sed -i "s/zebra=no/zebra=yes/" /etc/frr/daemons && sed -i "s/ospfd=no/ospfd=yes/" /etc/frr/daemons'
...
echo "### Passo 14: Testes de Conectividade (via OSPF com FRR) ###"
...
echo "### Passo 16: Verificação Final (OSPF com FRR) ###"
...
echo "=== Rotas e Status OSPF nos Roteadores (FRR) ==="
for router in $ROUTERS; do
  echo "--- $router (vtysh -c 'show ip route') ---"
  docker exec $router vtysh -c "show ip route"
  sleep 1
  echo "--- $router (vtysh -c 'show ip ospf neighbor') ---"
  docker exec $router vtysh -c "show ip ospf neighbor"
  sleep 1
  echo "--- $router (vtysh -c 'show ip ospf interface') ---"
  docker exec $router vtysh -c "show ip ospf interface"
  sleep 1
  # echo "--- $router (vtysh -c 'show ip ospf database') ---" # Descomente para mais detalhes
  # docker exec $router vtysh -c "show ip ospf database"
  # sleep 1
done
echo ""

echo "Configuração da topologia com OSPF (FRR) concluída."
